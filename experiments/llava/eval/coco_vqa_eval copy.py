...
from experiments.llava.eval.timing_utils import normalize_latency
...
class EvaluationSummary:
    ...
    normalized_avg_tokens_per_s: Optional[float] = None
    avg_generation_latency_s: Optional[float] = None
    avg_generation_tokens_per_s: Optional[float] = None
    normalized_avg_generation_tokens_per_s: Optional[float] = None
...
def load_model_and_processor(
    ...
    if args.fna_use_online_sampling:
        stream_size = args.fna_online_stream_size or args.fna_num_sample
        if stream_size <= 0:
            raise ValueError("--fna-online-stream-size must be > 0 when online sampling is enabled")
        fna_config["use_online_sampling"] = True
        fna_config["online_sampling_stream_size"] = stream_size
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--fna-use-online-sampling",
        action="store_true",
        help="Enable online sampling for autoregressive decoding phases",
    )
    parser.add_argument(
        "--fna-online-stream-size",
        type=int,
        default=None,
        help="Max number of cached online sampling tokens (defaults to --fna-num-sample)",
    )
...
def main() -> None:
    ...
    normalized_latency_pairs = [
        (normalized_latency, rec.num_generated_tokens)
        for rec in predictions
        for normalized_latency in [
            normalize_latency(rec.latency_s, rec.num_starting_tokens, rec.num_generated_tokens)
        ]
        if normalized_latency is not None and normalized_latency > 0
    ]
    normalized_generation_latency_pairs = [
        (normalized_latency, rec.num_generated_tokens)
        for rec in predictions
        if rec.generation_latency_s is not None
        for normalized_latency in [
            normalize_latency(rec.generation_latency_s, rec.num_starting_tokens, rec.num_generated_tokens)
        ]
        if normalized_latency is not None and normalized_latency > 0
    ]
    ...
    normalized_avg_tokens_per_s = None
    if normalized_latency_pairs:
        total_normalized_latency = float(sum(lat for lat, _ in normalized_latency_pairs))
        total_normalized_tokens = float(sum(tok for _, tok in normalized_latency_pairs))
        if total_normalized_latency > 0:
            normalized_avg_tokens_per_s = total_normalized_tokens / total_normalized_latency
    normalized_avg_generation_tokens_per_s = None
    if normalized_generation_latency_pairs:
        total_normalized_generation_latency = float(sum(lat for lat, _ in normalized_generation_latency_pairs))
        total_normalized_generation_tokens = float(sum(tok for _, tok in normalized_generation_latency_pairs))
        if total_normalized_generation_latency > 0:
            normalized_avg_generation_tokens_per_s = (
                total_normalized_generation_tokens / total_normalized_generation_latency
            )
    ...
    summary = EvaluationSummary(
        ...
        normalized_avg_tokens_per_s=normalized_avg_tokens_per_s,
        avg_generation_latency_s=avg_generation_latency,
        avg_generation_tokens_per_s=avg_generation_tokens_per_s,
        normalized_avg_generation_tokens_per_s=normalized_avg_generation_tokens_per_s,
